#!/usr/bin/env python3
# Minimize member/non-member gap: full train data, large batch, moderate noise, early stop.
...
tl=DataLoader(train,batch_size=1024,shuffle=True); te=DataLoader(test,batch_size=512)
m=SVHNCNN(); opt=torch.optim.SGD(m.parameters(),lr=0.1,momentum=0.9)
pe=PrivacyEngine()
m,opt,tl=pe.make_private(module=m,optimizer=opt,data_loader=tl,noise_multiplier=1.3,max_grad_norm=1.0)
def acc():
    m.eval(); c=t=0
    with torch.no_grad():
