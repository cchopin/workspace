#!/usr/bin/env python3
# Official walkthrough recipe: DP-SGD SVHN, target_epsilon=6 via make_private_with_epsilon.
...
dp_model=SVHNCNN().to(device); dp_model=ModuleValidator.fix(dp_model)
...
def acc():
    dp_model.eval(); c=t=0
    with torch.no_grad():
        for xb,yb in test_loader: c+=(dp_model(xb).argmax(1)==yb).sum().item(); t+=len(yb)
    dp_model.train(); return c/t
crit=nn.CrossEntropyLoss()
for ep in range(DP_EPOCHS):
    dp_model.train()
    for xb,yb in train_loader:
        optimizer.zero_grad(); crit(dp_model(xb),yb).backward(); optimizer.step()
    print(f"epoch {ep+1}/{DP_EPOCHS} acc {acc():.4f} eps {pe.get_epsilon(DELTA):.2f}",flush=True)
